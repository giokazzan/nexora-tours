...
function plantillaAdmin(d) {
  ...
  <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;background:#0d2d3f;color:#fff;border-radius:12px;overflow:hidden">
    ...
    <div style="padding:24px">
      <p style="font-size:22px;margin:0 0 16px;color:#3ecfc4"><strong>${d.vehiculoNombre}</strong></p>
      <table style="width:100%;font-size:14px;line-height:1.8">
        ...
        <tr><td style="color:#8ab2b0">Del</td><td style="text-align:right">${d.fechaInicio}</td></tr>
        <tr><td style="color:#8ab2b0">Al</td><td style="text-align:right">${d.fechaFin}</td></tr>
        <tr><td style="color:#8ab2b0">Días</td><td style="text-align:right">${d.dias}</td></tr>
        <tr><td style="color:#8ab2b0">Zona de entrega</td><td style="text-align:right">${d.zonaEntrega}</td></tr>
        <tr><td style="color:#8ab2b0">Total estimado</td><td style="text-align:right">$${d.totalEstimado} MXN</td></tr>
...
function plantillaCliente(d) {
  ...
  <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;background:#ffffff;color:#161514;border-radius:12px;overflow:hidden;border:1px solid #eee">
    ...
    <div style="padding:28px 24px">
      ...
      <p style="font-size:14px;color:#555;line-height:1.6">Recibimos tu solicitud de reserva de vehículo. Aquí está tu resumen:</p>
      <div style="background:#f7f7f5;border-radius:10px;padding:16px;margin:16px 0">
        <table style="width:100%;font-size:14px;line-height:1.9">
          <tr><td style="color:#888">Vehículo</td><td style="text-align:right"><strong>${d.vehiculoNombre}</strong></td></tr>
          <tr><td style="color:#888">Del</td><td style="text-align:right">${d.fechaInicio}</td></tr>
          <tr><td style="color:#888">Al</td><td style="text-align:right">${d.fechaFin}</td></tr>
          <tr><td style="color:#888">Días</td><td style="text-align:right">${d.dias}</td></tr>
          <tr><td style="color:#888">Zona de entrega</td><td style="text-align:right">${d.zonaEntrega}</td></tr>
          <tr><td style="color:#888">Total estimado</td><td style="text-align:right">$${d.totalEstimado} MXN</td></tr>
      ...
      <div style="background:#f7f7f5;border-radius:8px;padding:12px 16px;margin-top:16px;text-align:center">
        ...
        <a href="https://wa.me/529982168410?text=${encodeURIComponent('Hola, sigo esperando confirmación de mi reserva de vehículo, folio ' + d.folio)}" style="color:#0b9e99;font-weight:bold;text-decoration:none">Contacta a soporte por WhatsApp →</a></p>
      </div>
    </div>
  </div>`;
}
...
export default async function handler(req, res) {
  ...
  try {
    await enviar(ADMIN_EMAIL, `Nueva reserva de vehículo — ${d.vehiculoNombre} — ${d.folio}`, plantillaAdmin(d));
    resultados.admin = 'ok';
  } catch (e) { resultados.admin = e.message; }

  try {
    await enviar(d.email, `Tu reserva de vehículo en Nexora — folio ${d.folio}`, plantillaCliente(d));
